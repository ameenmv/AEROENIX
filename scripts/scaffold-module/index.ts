...
`
  ...
  const mockImportLine = hasMock
    ? `\nimport { mockFieldsFromKeys } from './mock'\n`
    : ''
  const mockConfigLines = hasMock
    ? `\n  // Toggle mock mode — set to false when the real API is ready\n  useMock: true,\n  mockFields: mockFieldsFromKeys(['id', 'created_at']),\n  // TODO: Add your entity field names to mockFieldsFromKeys()\n  // e.g. mockFieldsFromKeys(['id', 'name', 'email', 'status', 'created_at']),`
    : ''

  const serviceContent = `import { createService } from './createService'${mockImportLine}
import type { ${pascalName} } from '@/types'
import { ${ENDPOINT_KEY}_ENDPOINTS } from '@/modules/${camelName}/endpoints'

export const ${camelName}Service = createService<${pascalName}>(${ENDPOINT_KEY}_ENDPOINTS.LIST, {${mockConfigLines}
})
`
  ...
  // ─── 5. Mock Data File (optional — legacy, kept for backwards compat) ──
...
export const ${camelName}MockData = generateMockRecords(
