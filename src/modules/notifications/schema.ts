...
export function notificationFormSchema(t: TranslateFn) {
  return z.object({
    title: z.object({
      en: z
        .string({ required_error: t('notifications.validation.title_en_required', 'Title (EN) is required.') })
    ...
    content: z.object({
      en: z
        .string({ required_error: t('notifications.validation.content_en_required', 'Content (EN) is required.') })
    ...
    type: z
      .number({ required_error: t('notifications.validation.type_required', 'Notification type is required.') })
      .refine(
    ...
    audience_type: z
      .number({ required_error: t('notifications.validation.audience_required', 'Audience type is required.') })
      .refine(
        v => Object.values(AudienceType).includes(v as AudienceType),
        t('notifications.validation.audience_invalid', 'Invalid audience type.'),
      ),
    send_to: z
      .number({ required_error: t('notifications.validation.send_to_required', 'Send to is required.') })
      .refine(
