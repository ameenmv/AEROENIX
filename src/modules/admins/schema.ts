...
export function adminCreateSchema(t: TranslateFn) {
  return z
    .object({
      ...
      password: z
        .string({
          required_error: t('admins.validation.password_required', 'Password is required.'),
        })
        .min(8, t('admins.validation.password_min', 'Password must be at least 8 characters.')),
      password_confirmation: z
        .string({
          required_error: t(
            'admins.validation.password_confirmation_required',
      ...
      role_ids: z
        .array(
          z.number({ invalid_type_error: t('admins.validation.role_invalid', 'Invalid role.') }),
        )
        .optional()
        .default([]),
    })
