...
export function roleFormSchema(t: TranslateFn) {
  return z.object({
    display_name: z.object({
      en: z
        .string({ required_error: t('roles.validation.name_en_required', 'Role name (EN) is required.') })
      ...
      ar: z
        .string({ required_error: t('roles.validation.name_ar_required', 'Role name (AR) is required.') })
