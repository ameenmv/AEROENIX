...
import { decryptAndGetCookie, encryptAndSetCookie, removeCookie } from '@/utils/crypto'
...
export const useAuthStore = defineStore('auth', () => {
  ...
  async function login(credentials: LoginCredentials): Promise<string | null> {
    return withLoading(async () => {
      // Clear old state before login to prevent old token from being sent with OTP verify
      token.value = ''
