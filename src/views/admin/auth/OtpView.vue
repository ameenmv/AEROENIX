...
// Read the OTP challenge token from the route query (passed by login page)
const routeToken = computed(() => (route.query.token as string) || '')

// Guard: redirect to login if no token in route
if (!routeToken.value) {
...
async function handleVerify() {
  try {
    await verifyOtp(otp.value, routeToken.value)
    const lang = (route.params.lang as string) || 'en'
...
async function handleResend() {
  ...
  try {
    await resendOtp()
    // Token is auto-updated in store, countdowns restart automatically
    otp.value = ''
  ...
  <AuthLayout>
    ...
    >
      ...
      <CardContent class="!px-5 sm:!px-12 !pb-0">
        ...
        <form class="flex flex-col items-center gap-6" @submit.prevent="handleVerify">
          <!-- OTP Input -->
